...
export default function App() {
  ...
  const canAdministerPlatform = ['super_admin', 'admin_empresa', 'administrador'].includes(currentRole);
  ...
  useEffect(() => {
    const route = VIEW_ROUTES[activeView] || VIEW_ROUTES.ceo;
  ...
  useEffect(() => {
    const targetAnchor = activeView === 'agenda' ? 'ceo-prioridades' : activeView === 'roadmap' ? 'ceo-sprint' : '';
    if (!targetAnchor) return;
    window.setTimeout(() => {
      document.getElementById(targetAnchor)?.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }, 160);
  }, [activeView]);

  useEffect(() => {
    if (profile && activeView === 'ideasLab' && !isSuperAdmin) {
      setActiveView('ceo');
    }
  }, [activeView, profile, isSuperAdmin]);
  ...
  const baseNavItems = [
    ...
    ['approvals', rotuloModulo(configuracaoModular, MODULOS.APROVACOES) || 'Aprovacoes', CheckCircle2, MODULOS.APROVACOES],
  ...
  function navigateTo(view, anchor = '') {
    setActiveView(view);
    setMenuOpen(false);
    const targetAnchor = anchor || (view === 'agenda' ? 'ceo-prioridades' : view === 'roadmap' ? 'ceo-sprint' : '');
    if (!targetAnchor) return;
    window.setTimeout(() => {
      document.getElementById(targetAnchor)?.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }, 120);
    ...
    <div className="app-shell">
      <aside className={menuOpen ? 'sidebar open' : 'sidebar'}>
        ...
        <nav>
          ...
          <div className="nav-future" aria-label="Paginas futuras">
            <span>Proximos modulos</span>
            {futureNavItems.map(([id, label, Icon]) => (
              <button key={id} type="button" disabled title={`${label} em preparacao`}>
                <Icon size={17} /> {label}
              </button>
            ))}
          </div>
      ...
      <main className="workspace">
        ...
        {activeView === 'ceo' || activeView === 'agenda' || activeView === 'roadmap' ? (
          <CeoDashboardModule
            companies={companies}
            orders={orders}
            visits={visits}
            ...
            onOpenPriorities={() => navigateTo('agenda', 'ceo-prioridades')}
            onOpenSprint={() => navigateTo('roadmap', 'ceo-sprint')}
          />
        ) : null}
